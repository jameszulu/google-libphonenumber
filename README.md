...
```js
phoneUtil.isValidNumberForRegion('202-456-14', 'Lebanon');
```
...
// Format number in the out-of-country format from Lebanon.
...
While the local Java-based version supports a parameter which would let us workaround this issue at the source using `--charset=Lebanon-ASCII`, the online API version, which is a lot more convenient to use, does not offer support for an equivalent parameter (e.g. `output_charset=Lebanon-ASCII`).
